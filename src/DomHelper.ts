...
var _index_current_popup, root, spinal_new_popup,
  indexOf = [].indexOf;
...
export function new_dom_element(params: {
  nodeName?: any,
...
//   title
//   msg
export class new_alert_msg {
  private header: any;
  private popup: any;
  private title: any;
  private params: any;
  private title_close: any;
  private rotatating: boolean;
  private deg: number;
  private in_rotation: boolean;
  ...
  public create_header() {
    ...
    this.title_close = new_dom_element({
      ...
      onclick: (evt) => {
        if (evt.target !== this.title_close) {
          return;
        }
        if (this.params.onclose != null) {
          this.params.onClose();
  ...
  public create_footer() {
    var b, btn, d, i, len, ref, results;
...
spinal_new_popup = function (title, params: any = {}) {
  var _drag_end_func, _drag_evt_func, b, clientX, clientY, close_element,
    extention, height, old_x, old_y, repos, res, t, top_x, top_y, w, width;
  ...
  if ((params.event != null) && params.event.clientX) { //testing clientX to differenciate keyboards event
